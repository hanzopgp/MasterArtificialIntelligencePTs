...
class TutorialController(Controller):
	...
	## this helps the dispersion because since there is no noise, some robots follow other robots
	## forever and there is no convergence to a stable state
	def add_noise_to_converge(self, x, noise):
		return x + np.random.normal(0, noise, x.shape) 

	def get_discrete_value(self, x):
		return np.clip(self.add_noise_to_converge(x*0.5, noise=0.3) - 1, -1, 1)
	...
	def step(self):  # step is called at each time step
		...
		## we do the same than in agregation but we reverse the value in order to rotate in
		## the inverse side of the closest neighbor
		if camera_dist.min() == 1:
			self.set_rotation(-1)
		else:
			for i in range(self.nb_sensors):
				is_robot = self.get_robot_id_at(i) != -1
				is_wall = self.get_wall_at(i)
				## here we make dispersion of robots
				if is_robot:
					min_side = camera_dist.argmin()
					min_value = camera_dist.min()
					if min_side < 5: 
						self.set_rotation(- self.get_discrete_value(min_side))
					## this is not really needed but the dispersion is a bit faster
					else:
